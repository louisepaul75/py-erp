...
def update_legacy_artikel_ids(legacy_products_df):
    ...
    logger.info(f"Created mapping for {len(sku_to_id_map)} SKUs and {len(legacy_sku_to_id_map)} legacy SKUs")
    ...
    with transaction.atomic():
        for product in products:
            ...
            if artikel_id:
                ...
                not_found_count += 1
                logger.warning(f"Could not find legacy ID for product {product.sku} (legacy_sku: {product.legacy_sku})")
