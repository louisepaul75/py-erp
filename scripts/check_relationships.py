...
def check_relationships():
    ...
    print(f'Variants with parent: {variants_with_parent}/{total_variants} ({round(variants_with_parent/total_variants*100, 2)}%)')
    ...
    for var in variants_without_parent:
        ...
        if parent:
            print(f'  → Matching parent found: SKU: {parent.sku}, legacy_id: {parent.legacy_id}')
        else:
    ...
    if var_with_parent:
        ...
        print(f'Variant: {var_with_parent.sku}, Parent: {parent.sku}, Familie_: {var_with_parent.legacy_familie}, Parent legacy_id: {parent.legacy_id}')
