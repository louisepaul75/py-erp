        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    product_name = (
                        row["Bezeichnung"]
                        if "Bezeichnung" in row and pd.notna(row["Bezeichnung"])
                    ...
                    parent_product.base_sku = (
                        sku  # For parent products, base_sku is the same as sku
                    )
                    ...
                    if dry_run:
                        print(
                            f"Would {'update' if exists else 'create'} parent product: {sku} - {product_name}",
                        )
                        if exists:
                            updated += 1
                        else:
                    ...
                    errors += 1
                    print(f"Error processing record {index}: {e!s}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Import Artikel_Familie with minimal columns",
    )
    parser.add_argument(
        "--limit",
        type=int,
