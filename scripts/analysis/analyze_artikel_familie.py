...
try:
    ...
    def analyze_artikel_familie():
        ...
        try:
            ...
            if (
                "ParentID" in df.columns
                or "Parent_ID" in df.columns
                or "Eltern_ID" in df.columns
            ):
                parent_col = next(
                    (
                        col
                        for col in ["ParentID", "Parent_ID", "Eltern_ID"]
                ...
                if parent_col:
                    ...
                    print(
                        f"Number of root categories (no parent): {(df[parent_col].isna() | (df[parent_col] == 0)).sum()}",
                    )
                    max_depth = 1
