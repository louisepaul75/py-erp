...
def check_variant_parent_relationships():
    ...
    print(
        f"Variants with parent: {variants_with_parent} ({round(variants_with_parent / total_variants * 100, 2)}%)",
    )
    print(
        f"Variants without parent: {variants_without_parent} ({round(variants_without_parent / total_variants * 100, 2)}%)",
    )
    ...
    if variants_without_parent > 0:
        ...
        print(
            f"- {variants_without_parent - all_potential_fixes} have no matching parent",
        )
    ...
    if var_with_parent:
        ...
        print(
            f"Familie matches Legacy ID: {'✓' if var_with_parent.legacy_familie == parent.legacy_id else '✗'}",
        )
