...
try:
    ...
    from django.db import DataError, IntegrityError, connection, connections, models
    from django.db.models import Count, F, Q

    from pyerp.core.validators import ValidationResult
except ImportError as e:
    print(f"Error importing Django modules: {e!s}")
...
class DatabaseValidator:
    ...
    def check_database_connection(self):
        ...
        try:
            ...
            self.log(f"Database connection failed: {e!s}", "error")
            return False
    ...
    def list_tables(self):
        ...
        try:
            ...
            self.log(f"Error listing tables: {e!s}", "error")
            return []
    ...
    def check_table_structure(self, table_name):
        ...
        try:
            ...
            self.log(
                f"Error checking table structure for '{table_name}': {e!s}",
                "error",
            )
            return []
            ...
            with connection.cursor() as cursor:
                if connection.vendor == "postgresql":
                    ...
                    for fk in foreign_keys:
                        ...
                        if broken_count > 0:
                            self.log(
                                f"Found {broken_count} broken foreign key references: {table_name}.{column_name} -> {foreign_table}.{foreign_column}",
                                "error",
                            )

                    return foreign_keys
                self.log(
                    "Foreign key checks only implemented for PostgreSQL",
                    "warning",
                )
                return []
        except Exception as e:
            self.log(f"Error checking foreign keys for '{table_name}': {e!s}", "error")
            return []
        ...
        try:
            with connection.cursor() as cursor:
                ...
                for column in not_null_columns:
                    ...
                    if null_count > 0:
                        self.log(
                            f"Found {null_count} NULL values in NOT NULL column: {table_name}.{column}",
                            "error",
                        )
        except Exception as e:
            self.log(
                f"Error checking NULL constraints for '{table_name}': {e!s}",
                "error",
            )
            ...
            with connection.cursor() as cursor:
                if connection.vendor == "postgresql":
                    ...
                    for column in unique_columns:
                        ...
                        if duplicates:
                            self.log(
                                f"Found duplicate values in unique column: {table_name}.{column}",
                                "error",
                            )
                            if self.verbose:
                                for dup in duplicates:
                                    self.log(
                                        f"  - Value '{dup[0]}' appears {dup[1]} times",
                                    )
        except Exception as e:
            self.log(
                f"Error checking unique constraints for '{table_name}': {e!s}",
                "error",
            )
                        ...
                        if data_type in ("numeric", "decimal"):
                            ...
                            if min_val is not None and max_val is not None:
                                if abs(min_val) > 1e9 or abs(max_val) > 1e9:
                                    self.log(
                                        f"Found extreme numeric values in {table_name}.{column}: range [{min_val}, {max_val}]",
                                        "warning",
                                    )
        except Exception as e:
            self.log(f"Error checking data types for '{table_name}': {e!s}", "error")
    ...
    def check_index_usage(self):
        ...
        try:
            ...
            self.log(f"Error checking index usage: {e!s}", "warning")

    def run_validation(self):
        """Run all validation checks."""
        start_time = datetime.now()
        self.log(
            f"Starting database validation at {start_time.strftime('%Y-%m-%d %H:%M:%S')}",
        )

        # Check database connection
        if not self.check_database_connection():
            self.log(
                "Cannot proceed with validation due to database connection failure",
                "error",
            )
        ...
        return {
            ...
            "summary": "\n".join(
                [
                    ...
                    if "Database validation completed" in msg["message"]
                ],
            ),
