...
def convert_unittest_to_pytest(content):
    ...
    content = re.sub(r"self\.assertEqual\((.*?), (.*?)\)", r"assert \1 == \2", content)
    ...
    content = re.sub(r"self\.assertIn\((.*?), (.*?)\)", r"assert \1 in \2", content)
...
def convert_setup_to_fixtures(setup_content):
    ...
    assignments = re.findall(r"self\.(\w+) = (.*?)$", setup_content, re.MULTILINE)
...
def main():
    ...
    test_files = [
        f for f in source_dir.glob("test_*.py") if f.name != "test_settings.py"
    ]
    ...
    print(
        "Remember to update imports and fix any issues with the automated conversion."
    )
