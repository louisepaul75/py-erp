        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    legacy_id = row['UID'] if 'UID' in row and pd.notna(row['UID']) else None
                    familie_ = row['Familie_'] if 'Familie_' in row and pd.notna(row['Familie_']) else None
                    nummer = row['Nummer'] if 'Nummer' in row and pd.notna(row['Nummer']) else None
                    alte_nummer = row['alteNummer'] if 'alteNummer' in row and pd.notna(row['alteNummer']) else None
                    ...
                    parent = None
                    if familie_:
                        parent_candidates = ParentProduct.objects.filter(legacy_id=familie_)
                    ...
                    if not parent:
                        parent_not_found += 1
                        print(f"Parent product not found for variant: {alte_nummer} - Skipping")
                        continue
                    ...
                    variant_data = {
                        ...
                        'variant_code': variant_code if 'variant_code' in locals() else '',
                    ...
                    with transaction.atomic():
                        variant, created_status = VariantProduct.objects.update_or_create(
                            legacy_id=legacy_id,
                    ...
                    errors += 1
                    print(f"Error processing variant {index}: {str(e)}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Import Artikel_Variante with minimal columns')
    ...
    print(f"Import completed: {created} created, {updated} updated, {skipped} skipped, {parent_not_found} missing parents") 