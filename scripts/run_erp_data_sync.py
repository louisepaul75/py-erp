...
def import_artikel_familie(limit=None, dry_run=False):
    ...
    try:
        ...
        df = client.fetch_table('Artikel_Familie', top=limit or 1000, new_data_only=False)
...
def import_artikel_variante(limit=None, dry_run=False):
    ...
    try:
        ...
        # Fetch data from the legacy ERP
        df = client.fetch_table('Artikel_Variante', top=limit or 1000, new_data_only=False)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Import ERP data (Artikel_Familie and Artikel_Variante)')
    parser.add_argument('--dry-run', action='store_true', help='Simulate without making changes')
    parser.add_argument('--limit', type=int, help='Limit the number of records to import')
    parser.add_argument('--skip-familie', action='store_true', help='Skip importing Artikel_Familie')
    parser.add_argument('--skip-variante', action='store_true', help='Skip importing Artikel_Variante')
