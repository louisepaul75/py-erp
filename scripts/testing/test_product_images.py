    ...
    try:
        ...
        while total_pages is None or page <= total_pages:
            ...
            if total_pages is None:
                ...
                logger.info(f"Found {count} total records, {total_pages} pages to process")
            ...
            # If we've found at least one image and not in verbose mode, we can stop
        ...
        if not product_images:
            ...
            for i, item in enumerate(product_images):
                ...
                if verbose:
                    ...
                    # Check original file
                    if item.get('original_file') and 'file_url' in item['original_file']:
                        urls.append({
                            'type': item['original_file'].get('type', 'unknown'),
                            'format': item['original_file'].get('format', 'unknown'),
                    ...
                    for url_info in urls:
                        print(f"  - {url_info['type']} ({', '.join(str(r) for r in url_info.get('resolution', []) if r)}): {url_info['url']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test fetching images for a specific product')
