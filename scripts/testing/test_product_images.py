    ...
    try:
        ...
        while total_pages is None or page <= total_pages:
            ...
            if total_pages is None:
                ...
                logger.info(
                    f"Found {count} total records, {total_pages} pages to process")  # noqa: E501
            ...
            if product_images and not verbose:
                logger.info(
                    f"Found {
                        len(product_images)} images, stopping search")
                break

        if not product_images:
            logger.warning(
                f"No images found for article number: {article_number}")
        else:
            logger.info(
                f"✅ Found {
                    len(product_images)} images for article: {article_number}")
            ...
            for i, item in enumerate(product_images):
                ...
                if verbose:
                    ...
                    # Check original file
                    if item.get(
                            'original_file') and 'file_url' in item['original_file']:  # noqa: E501
                        urls.append({
                            'type': item['original_file'].get('type', 'unknown'),  # noqa: E501
                            'format': item['original_file'].get('format', 'unknown'),  # noqa: E501
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        'article_number',
        help='The article number to search for')
    parser.add_argument(
        '--verbose',
        '-v',
        action='store_true',
