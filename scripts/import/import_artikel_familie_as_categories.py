...
try:
    ...
    def import_artikel_familie_as_categories(limit=None, update_existing=True, dry_run=False):
        """
            ...
            try:
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        family_name = row['Bezeichnung'] if pd.notna(row['Bezeichnung']) else "Unnamed Family"
                        ...
                        if not exists or update_existing:
                            ...
                            if exists:
                                updated += 1
                                print(f"Updated category '{family_name}' (ID: {familie_id})")
                            else:
                                created += 1
                                print(f"Created category '{family_name}' (ID: {familie_id})")
                        else:
                            skipped += 1
                            print(f"Skipped existing category '{family_name}' (ID: {familie_id})")
    ...
    if __name__ == "__main__":
        ...
        parser = argparse.ArgumentParser(description="Import Artikel_Familie data as product categories")
        parser.add_argument('--limit', type=int, default=0, help='Limit the number of records to import')
        parser.add_argument('--update', action='store_true', help='Update existing categories')
