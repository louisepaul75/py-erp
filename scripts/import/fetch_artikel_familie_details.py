    ...
    def fetch_artikel_familie_details():
        ...
        try:
            ...
            if 'Familie_' in av_df.columns:
                ...
                print(f"Top 10 most common Familie_ values:")
                ...
                try:
                    ...
                    for i, familie_id in enumerate(sample_ids):
                        print(f"Fetching details for Familie ID {i+1}/{len(sample_ids)}: {familie_id}")
                        try:
                    ...
                    if familie_details:
                        ...
                        print(f"Fields in Artikel_Familie: {', '.join(sorted(all_keys))}")
                        
                        # Print sample of family details
                        print("\nSample of Familie details:")
                        for i, (id, details) in enumerate(list(familie_details.items())[:3]):
                            print(f"\nFamilie {i+1}:")
                            for key, value in details.items():
                                print(f"  {key}: {value}")
                        
                        # Try to find hierarchical structure
                        if any('Parent' in key for key in all_keys) or any('parent' in key.lower() for key in all_keys):
                            print("\nHierarchical structure detected!")
                            parent_key = next((k for k in all_keys if 'parent' in k.lower()), None)
                            if parent_key:
