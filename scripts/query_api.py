...
def main():
    ...
    first_page = client._make_request("all-files-and-articles/", params={"page": 1, "page_size": 50})
    ...
    for idx, (result_idx, result) in enumerate(results_with_articles):
        ...
        if 'exported_files' in result:
            ...
            if result.get('exported_files'):
                print(f"First exported file URL: {result['exported_files'][0].get('image_url')}")
