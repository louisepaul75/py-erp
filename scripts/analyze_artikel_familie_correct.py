...
try:
    ...
    def analyze_artikel_familie():
        ...
        try:
            ...
            for possible_name in ['Name', 'Bezeichnung', 'name', 'bezeichnung', 'Description', 'description']:
            ...
            for possible_parent in ['ParentID', 'Parent_ID', 'Eltern_ID', 'parent', 'Parent', 'parent_id', 'ParentKey', 'Eltern']:
            ...
            if parent_field:
                ...
                non_zero_parents = non_empty_parents[non_empty_parents != 0] if non_empty_parents.dtype in ['int64', 'float64'] else non_empty_parents
                parent_counts = Counter(non_zero_parents)
                
                print(f"Number of unique parent IDs: {len(parent_counts)}")
                root_count = (df[parent_field].isna() | (df[parent_field] == 0)).sum() if df[parent_field].dtype in ['int64', 'float64'] else df[parent_field].isna().sum()
                print(f"Number of root categories (no parent): {root_count}")
                
                # Build a hierarchy structure
                hierarchy = defaultdict(list)
                id_field = next((col for col in ['__KEY', 'ID', 'UID', 'id', 'uid'] if col in df.columns), None)
                ...
                if id_field:
                    ...
                    for i, row in df.iterrows():
                        ...
                        if not pd.isna(parent_id) and (parent_id != 0 if df[parent_field].dtype in ['int64', 'float64'] else True):
                    ...
                    def get_depth(parent_id, visited=None):
                        ...
                        if parent_id not in hierarchy:
                            return 1
                        return 1 + max((get_depth(child, visited.copy()) for child in hierarchy[parent_id]), default=0)
                    
                    root_ids = []
                    for i, row in df.iterrows():
                        if pd.isna(row[parent_field]) or (row[parent_field] == 0 if df[parent_field].dtype in ['int64', 'float64'] else False):
                            root_ids.append(row[id_field])
                    
                    max_depth = max((get_depth(root_id) for root_id in root_ids), default=1)
                    ...
                    print(f"Max children per parent: {max(child_counts) if child_counts else 0}")
                    
                    # Print top-level categories
                    if name_field and root_ids:
                        print("\nTop-level Categories:")
                        for i, root_id in enumerate(root_ids[:10]):  # Show first 10 root categories
                            root_name = df.loc[df[id_field] == root_id, name_field].iloc[0] if not df.loc[df[id_field] == root_id, name_field].empty else "Unknown"
                            children_count = len(hierarchy.get(root_id, []))
                            print(f"  {i+1}. {root_name} (ID: {root_id}) - {children_count} direct children")
