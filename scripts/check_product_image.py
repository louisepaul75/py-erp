...
def check_product_images(sku):
    ...
    for img in all_images:
        print(
            f"- Type: {img.image_type}, Front: {img.is_front}, Primary: {img.is_primary}",
        )

    # Find Produktfoto images with front=True
    produktfotos = product.images.filter(
    ...
    selected_image = product.images.filter(
        image_type__iexact="Produktfoto",
        is_front=True,
    ).first()
    ...
    if selected_image:
        ...
        print(
            f"- Type: {selected_image.image_type}, Front: {selected_image.is_front}, Primary: {selected_image.is_primary}",
        )
