    ...
    <div class="mb-6 bg-white rounded-lg shadow p-4">
      <div class="flex flex-col md:flex-row gap-4">
        ...
        <div class="flex-1">
          <div class="relative">
            <input
              ...
              @input="applyFilters"
            />
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute left-3 top-2.5 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        >
          ...
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-2" :class="{'rotate-180': showAdvancedSearch}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div v-if="showAdvancedSearch" class="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
        ...
        <div>
          ...
          <div class="flex space-x-4">
            <label class="inline-flex items-center">
              <input type="radio" v-model="filters.customerType" value="all" class="form-radio" @change="applyFilters" />
              <span class="ml-2">Alle</span>
            </label>
            <label class="inline-flex items-center">
              <input type="radio" v-model="filters.customerType" value="b2b" class="form-radio" @change="applyFilters" />
              <span class="ml-2">Partner (B2B)</span>
            </label>
            <label class="inline-flex items-center">
              <input type="radio" v-model="filters.customerType" value="b2c" class="form-radio" @change="applyFilters" />
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tr>
              ...
              >
                <div class="flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 text-gray-400 cursor-move" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  {{ column.label }}
                  <svg v-if="sortColumn === column.key" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1" :class="{'rotate-180': !sortAscending}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="customer in paginatedCustomers" :key="customer.id" class="hover:bg-gray-50">
              <td v-for="column in displayColumns" :key="column.key" class="px-6 py-4 whitespace-nowrap">
                ...
                <template v-else-if="column.key === 'type'">
                  <span 
                    ...
                    :class="customer.type === 'b2b' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'"
                  >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="text-indigo-600 hover:text-indigo-900 mr-3" @click="viewCustomerDetails(customer)">
                  Details
                </button>
                <div class="relative inline-block">
                  <button class="text-gray-600 hover:text-gray-900" @click="toggleActionMenu(customer.id)">
                  ...
                  <div v-if="activeActionMenu === customer.id" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10">
                    <div class="py-1">
                      <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" @click.prevent="editCustomer(customer)">Bearbeiten</a>
                      <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" @click.prevent="viewOrders(customer)">Bestellungen anzeigen</a>
                      <a href="#" class="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100" @click.prevent="deleteCustomer(customer)">Löschen</a>
                    </div>
                  </div>
                </div>
              </td>
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p class="text-sm text-gray-700">
              Zeige <span class="font-medium">{{ (currentPage - 1) * pageSize + 1 }}</span> bis <span class="font-medium">{{ Math.min(currentPage * pageSize, filteredCustomers.length) }}</span> von <span class="font-medium">{{ filteredCustomers.length }}</span> Einträgen
            </p>
          </div>
          <div>
...
onMounted(() => {
  ...
  const countries = ['Deutschland', 'Österreich', 'Schweiz', 'Frankreich', 'Italien', 'Niederlande', 'Belgien'];
  const now = new Date();
...
const filteredCustomers = computed(() => {
  ...
  return customers.value.filter(customer => {
    ...
    // Advanced search
    if (advancedSearch.value.name && !customer.name.toLowerCase().includes(advancedSearch.value.name.toLowerCase())) {
      return false;
    }
    
    if (advancedSearch.value.email && !customer.email.toLowerCase().includes(advancedSearch.value.email.toLowerCase())) {
      return false;
    }
    
    if (advancedSearch.value.city && !customer.city.toLowerCase().includes(advancedSearch.value.city.toLowerCase())) {
      return false;
    }
    
    if (advancedSearch.value.postalCode && !customer.postalCode.includes(advancedSearch.value.postalCode)) {
      return false;
    }

    if (advancedSearch.value.documentNumber && !customer.customerNumber.includes(advancedSearch.value.documentNumber)) {
      return false;
    }

    if (advancedSearch.value.shopNumber && !customer.shopNumber.includes(advancedSearch.value.shopNumber)) {
      return false;
    }
