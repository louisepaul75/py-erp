...
def fix_variant_foreign_key():
    ...
    with connection.cursor() as cursor:
        ...
        try:
            ...
            if orphaned_count > 0:
                print(f"WARNING: Found {orphaned_count} orphaned records. Setting parent_id to NULL for these records.")
                cursor.execute(
                    """
                    UPDATE products_variantproduct v
                    SET parent_id = NULL
                    WHERE parent_id IS NOT NULL AND NOT EXISTS (
                        SELECT 1 FROM products_parentproduct p WHERE v.parent_id = p.id
                    );
                    """
                )
            ...
            if constraint_count > 0:
                print(f"Verification successful: Found {constraint_count} foreign key constraint(s) for parent_id.")
                return True
            else:
