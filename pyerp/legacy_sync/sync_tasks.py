...
def sync_entity(entity_type: str, new_only: bool = True) -> dict[str, Any]:
    ...
    try:
        mapping_config = EntityMappingConfig.objects.get(
            entity_type=entity_type,
            is_active=True,
        )
    except EntityMappingConfig.DoesNotExist:
        logger.error(f"No active mapping configuration found for {entity_type}")
        error_message = f"No active mapping configuration found for {entity_type}"
    ...
    try:
        ...
        logger.info(
            f"Fetched {stats['total_fetched']} {entity_type} records from legacy system",
        )
            ...
            try:
                with transaction.atomic():
                    ...
                    if entity_type == "variant_product":
                        try:
                            if not EntityMapping.objects.filter(
                                entity_type=entity_type,
                                legacy_id=legacy_id,
                            ).exists():
                                ...
                                stats["created"] += 1
                                logger.debug(
                                    f"Created {entity_type} {legacy_id} -> {entity.id}",
                                )
                            ...
                            stats["updated"] += 1
                            logger.debug(
                                f"Updated {entity_type} {legacy_id} -> {entity.id}",
                            )
                            continue  # Skip the regular create/update flow
                        except Exception as e:
                            logger.error(
                                f"Error in custom mapping for {entity_type} {legacy_id}: {e}",
                            )
                            stats["errors"] += 1
                            continue
                    ...
                    try:
                        ...
                        stats["created"] += 1
                        logger.debug(
                            f"Recreated {entity_type} {legacy_id} -> {entity.id}",
                        )

            except Exception as e:
                stats["errors"] += 1
                logger.error(
                    f"Error processing {entity_type} {legacy_data.get('id', 'unknown')}: {e}",
                )
