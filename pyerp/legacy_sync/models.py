...
class EntityMappingConfig(models.Model):
    ...
    new_model = models.CharField(
        max_length=100,
        help_text=_(
            "Full path to the model class in the new system (e.g., pyerp.products.models.Product)",
        ),
    )
            ...
            if legacy_field in legacy_data:
                ...
                if transform_func:
                    if transform_func == "to_uppercase":
                        ...
                        transformed_value = (
                            int(float(legacy_value)) if legacy_value else 0
                        )
...
class TransformationFunction(models.Model):
    ...
    code = models.TextField(
        help_text=_(
            "Python code for the transformation function. Should take a single value parameter and return the transformed value.",
        ),
    )
    is_active = models.BooleanField(
        default=True,
