...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Starting synchronization of {entity_type} data"),
        )
        ...
        try:
            ...
            if force:
                self.stdout.write(
                    self.style.WARNING(f"Error during synchronization: {e}"),
                )
    ...
    def sync_all(self, new_only, force):
        ...
        """
        self.stdout.write(
            self.style.SUCCESS("Starting synchronization of all entity types"),
        )
        self.stdout.write(f"New only: {new_only}, Force: {force}")
        ...
        entity_types = EntityMappingConfig.objects.filter(is_active=True).values_list(
            "entity_type",
            flat=True,
        )
        ...
        if not entity_types:
            self.stdout.write(
                self.style.WARNING("No active entity types found for synchronization"),
            )
            return

        self.stdout.write(
            f"Found {len(entity_types)} active entity types: {', '.join(entity_types)}",
        )
        ...
        for entity_type in entity_types:
            try:
                ...
                if force:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Error during {entity_type} synchronization: {e}",
                        ),
                    )
                    self.stdout.write(
                        self.style.WARNING("Continuing due to --force flag"),
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(
    ...
    def list_entity_types(self):
        ...
        """
        self.stdout.write(
            self.style.SUCCESS("Available entity types for synchronization:"),
        )
        ...
        if not entity_configs:
            self.stdout.write(
                self.style.WARNING("No entity mapping configurations found"),
            )
            return
        ...
        for config in entity_configs:
            ...
            self.stdout.write(
                f"- {config.entity_type} ({status}): {config.legacy_table} -> {config.new_model}",
            )
