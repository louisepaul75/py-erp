...
try:
    ...
    logger.error(
        "Make sure the direct_api module is available and correctly configured",
    )
    sys.exit(1)
...
class DirectAPIClientWASID(DirectAPIClient):
            ...
            with open(self.cookie_file) as f:
                ...
                if "value" in cookie_data:
                    ...
                    if "created_at" in cookie_data:
                        try:
                            self.session_created_at = datetime.fromisoformat(
    ...
    def invalidate_and_reload_cookie(self):
        ...
        if is_session_limit_reached():
            logger.warning(
                "Not invalidating session because the global session limit has been reached",
            )
            return False
        ...
        if success and old_session_id == self.session_id:
            logger.warning(
                "Session ID did not change after invalidation - server may be limiting sessions",
            )
            return False
                ...
                if not is_session_limit_reached():
                    try:
                        ...
                        if "session limit" in str(e).lower() or "402" in str(e):
                            ...
                            error_msg = f"Cannot create a new session: {e!s}"
                        else:
...
def main():
    ...
    try:
        ...
        if args.all:
            ...
            logger.info(
                f"Fetching up to {args.top} records from '{args.table_name}' (skip: {args.skip})",
            )
...
def fetch_all_records(client, table_name, filter_query=None):
    ...
    try:
        while True:
            if is_session_limit_reached():
                logger.warning(
                    "Stopping data fetch because the global session limit has been reached",
                )
                break
            ...
            try:
                if skip in problematic_skip_values:
                    logger.warning(
                        f"Skipping problematic offset {skip} that consistently returns 402 errors",
                    )
                    skip += top
                    continue
                ...
                if "402" in error_str or "session limit" in error_str.lower():
                    logger.warning(
                        f"Received 402 error (too many sessions) at offset {skip}",
                    )

                    # Set the global session limit flag
                    set_session_limit_reached(True)
                    logger.warning(
                        "Hit session limit - will not create any new sessions (global flag set)",
                    )

                    # If we've already fetched a significant number of records, stop and return what we have
                    if len(all_data) > 0:
                        logger.warning(
                            f"We've already collected {sum(len(df) for df in all_data)} records. "
                            "Stopping to avoid creating more sessions.",
                        )
                        break
                    ...
                    if retry_count < max_retries:
                        ...
                        logger.warning(
                            f"Too many sessions error detected. Waiting {wait_time} seconds before retry {retry_count}/{max_retries}",
                        )
                        ...
                        continue
                    logger.error(
                        f"Failed to fetch data after {max_retries} retries at offset {skip} - too many sessions",
                    )
                    ...
                    if not all_data:
                        logger.error(
                            "Unable to fetch any data due to session limits. Exiting.",
                        )
                        break
                ...
                if retry_count < max_retries:
                    ...
                    wait_time = 5 * (
                        2**retry_count
                    )  # Increased base wait time: 10, 20, 40 seconds...
                    logger.warning(
                        f"Error: {error_str}. Retrying in {wait_time} seconds (attempt {retry_count}/{max_retries})",
                    )
                    time.sleep(wait_time)
                    continue
                raise
        ...
        if len(all_data) > 0:
            logger.warning(
                f"Returning {sum(len(df) for df in all_data)} records that were successfully fetched before the error",
            )
        else:
            raise
    ...
    if all_data:
        ...
        if last_successful_skip > 0:
            logger.info(
                f"Successfully fetched records up to offset {last_successful_skip + top - 1}",
            )
...
def test_session_management():
    ...
    logger.info(
        f"Created test client with session ID: {client.session_id[:10] if client.session_id else 'None'}",
    )
    ...
    try:
        ...
        logger.info(
            f"Request succeeded despite session limit flag! Retrieved {len(data)} records",
        )
    except Exception as e:
    ...
    try:
        data = fetch_all_records(client, table_name, filter_query=None)
        logger.info(
            f"fetch_all_records with session limit returned {len(data)} records",
        )
    except Exception as e:
