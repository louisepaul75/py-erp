...
in the legacy ERP system using the direct_api module with simplified session management.
...
COOKIE_FILE_PATH = os.path.join(
    os.path.dirname(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'auth.py'))),
...
class SimpleAPIClient:
    ...
    def __init__(self, environment='live'):
        ...
        if environment not in API_ENVIRONMENTS:
            ...
            logger.error(f"Invalid environment: '{environment}'. Available environments: {available_envs}")
            raise ValueError(f"Invalid environment: '{environment}'. Available: {available_envs}")
        ...
        if 'base_url' not in API_ENVIRONMENTS[environment]:
            ...
            logger.error(f"API_ENVIRONMENTS['{environment}']: {API_ENVIRONMENTS[environment]}")
        ...
        try:
            with open(COOKIE_FILE_PATH, 'r') as f:
                ...
                if 'value' in cookie_data:
                    ...
                    # Provide safe version of cookie for logging (only show first/last few chars)
                    safe_id = f"{self.session_id[:5]}...{self.session_id[-5:]}" if len(self.session_id) > 10 else self.session_id
    ...
    def _clear_cookies(self, cookie_name):
        ...
        try:
            if cookie_name in self.session.cookies:
                logger.info(f"Clearing existing {cookie_name} cookies to prevent duplicates")
                ...
                remaining = sum(1 for c in self.session.cookies if c.name == cookie_name)
                if remaining > 0:
                    logger.warning(f"Failed to clear all {cookie_name} cookies, {remaining} remain")
                else:
    ...
    def _log_response_cookies(self, response, prefix="Response cookies"):
        ...
        if 'Set-Cookie' in response.headers:
            logger.info(f"Raw Set-Cookie header: {response.headers['Set-Cookie'][:100]}...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if wasid_cookie:
                    ...
                    logger.info(f"Set single new cookie WASID4D with value from response")
    ...
    def login(self):
        ...
        try:
            ...
            if wasid_cookie:
                ...
                self.session_id = wasid_cookie
                self.session.cookies.set('WASID4D', wasid_cookie)
                safe_id = f"{wasid_cookie[:5]}...{wasid_cookie[-5:]}" if len(wasid_cookie) > 10 else wasid_cookie
                ...
                safe_id = f"{dsid_cookie[:5]}...{dsid_cookie[-5:]}" if len(dsid_cookie) > 10 else dsid_cookie
    ...
    def fetch_table(self, table_name, top=100, skip=0, all_records=False,
                    filter_query=None, new_data_only=True, date_created_start=None):
        """
        ...
        if all_records:
            ...
            if 'value' in data and isinstance(data['value'], list):
                ...
                if '__ENTITIES' in data and isinstance(data['__ENTITIES'], list):
                    ...
                    try:
                        df = pd.DataFrame([data])
                        logger.info(f"Retrieved 1 record from direct dict response")
                        return df
                    except Exception as e:
    ...
    def _fetch_all_records(self, url, filter_query=None, new_data_only=True, date_created_start=None):
        ...
        while True:
            ...
            try:
                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 402 and "Maximum number of sessions" in response.text:
                        if retry_count < max_retries:
                            logger.warning("Maximum sessions error detected. Attempting to login with a new session...")
                            # Clear cookies and login to get a new session
                            self.session.cookies.clear()
                            if self.login():
                                retry_count += 1
                                logger.info(f"Re-login successful. Retrying request (attempt {retry_count}/{max_retries})")
                                continue
                        error_msg += ": Maximum number of sessions reached and retry failed"
                ...
                for cookie in response.cookies:
                    if cookie.name == 'WASID4D' and cookie.value != self.session_id:
                        logger.info("Received new session cookie in paginated response, updating...")
                ...
                if total_fetched == 0:
                    logger.info(f"First response structure: {list(data.keys()) if isinstance(data, dict) else type(data)}")
                ...
                if 'value' in data and isinstance(data['value'], list):
                    ...
                    all_data.extend(records)
                    total_fetched += len(records)
                    logger.info(f"Fetched {len(records)} records from __ENTITIES (total: {total_fetched})")
                    ...
                    logger.info(f"Fetched {len(data)} records from direct list response (total: {total_fetched})")
                    break
                else:
