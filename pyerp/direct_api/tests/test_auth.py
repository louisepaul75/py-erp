...
from pyerp.direct_api.settings import API_SESSION_EXPIRY, API_SESSION_REFRESH_MARGIN
...
class TestSession(TestCase):
    ...
    def test_is_valid_expired(self):
        ...
        self.session.cookie = 'test-cookie'
        self.session.created_at = datetime.datetime.now() - datetime.timedelta(seconds=API_SESSION_EXPIRY + 1)
        self.session.expires_at = self.session.created_at + datetime.timedelta(seconds=API_SESSION_EXPIRY)
        self.assertFalse(self.session.is_valid())
...
class TestSessionPool(TestCase):
    ...
    def test_get_session_new(self, mock_session_class):
        ...
        mock_session.expires_at = mock_session.created_at + datetime.timedelta(seconds=API_SESSION_EXPIRY)
...
class TestGlobalFunctions(TestCase):
    ...
    def test_get_session(self, mock_get_session):
        ...
        # Verify the pool's get_session was called and the result passed through
