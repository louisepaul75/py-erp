...
class StammLagerorteTransformer(BaseTransformer):
    ...
    def transform(self, source_data: List[Dict]) -> List[Dict]:
        ...
        for record in source_data:
            ...
            legacy_id = (
                str(record.get(legacy_id_field))
                if record.get(legacy_id_field) is not None
                else None
            )
            ...
            for target_field, source_field in self.field_mappings.items():
                ...
                if source_field in record and record[source_field] is not None:
                    ...
                    if (
                        target_field in ["unit", "compartment", "shelf"]
            ...
            if "unit" not in transformed and "Regal" in record:
                try:
                    transformed["unit"] = (
                        str(int(record["Regal"])) if record["Regal"] is not None else ""
                    )
                except (ValueError, TypeError):
                    transformed["unit"] = (
                        str(record["Regal"]) if record["Regal"] else ""
                    )
            if "compartment" not in transformed and "Fach" in record:
                try:
                    transformed["compartment"] = (
                        str(int(record["Fach"])) if record["Fach"] is not None else ""
                    )
                except (ValueError, TypeError):
                    transformed["compartment"] = (
                        str(record["Fach"]) if record["Fach"] else ""
                    )
            if "shelf" not in transformed and "Boden" in record:
                try:
                    transformed["shelf"] = (
                        str(int(record["Boden"])) if record["Boden"] is not None else ""
                    )
                except (ValueError, TypeError):
                    transformed["shelf"] = (
                        str(record["Boden"]) if record["Boden"] else ""
                    )
            ...
            if location_key in used_combinations:
                ...
                if "unit" in transformed:
                    transformed["unit"] = (
                        f"{transformed['unit']}_" f"{used_combinations[location_key]}"
                    )
            else:
                used_combinations[location_key] = 1
            ...
            if "name" not in transformed:
                ...
                if name_parts:
                    transformed["name"] = "-".join(str(part) for part in name_parts)
                else:
...
class BoxTypeTransformer(BaseTransformer):
    ...
    def transform(self, source_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        schuettentypen_records = [
            record for record in source_data if record.get("scope") == "Schüttentypen"
        ]
        ...
        for record in schuettentypen_records:
            if "data_" not in record or not record["data_"]:
                logger.warning(f"Record {record.get('id')} has no data_ field")
                continue
    ...
    def _transform_box_type(self, box_type: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if not box_type.get("Type"):
            logger.warning(f"Skipping box type with no Type field: {box_type}")
            return None
        ...
        if box_type.get("Box_Länge"):
            dimensions.append(f"Length: {box_type.get('Box_Länge')}mm")
        if box_type.get("Box_Breite"):
            dimensions.append(f"Width: {box_type.get('Box_Breite')}mm")
        if box_type.get("Box_Höhe"):
            dimensions.append(f"Height: {box_type.get('Box_Höhe')}mm")
        ...
        if (
            box_type.get("Trenner_Gewicht")
            and float(box_type.get("Trenner_Gewicht", 0)) > 0
        ):
            description += f" - Divider Weight: {box_type.get('Trenner_Gewicht')}g"
        ...
        weight_capacity = self._convert_to_decimal(
            box_type.get("Box_Gewicht"), unit_conversion=0.001, round_to=2
        )
    ...
    def _convert_to_decimal(self, value, unit_conversion=0.1, round_to=2) -> Decimal:
        """
...
class BoxTransformer(BaseTransformer):
    ...
    def transform(self, source_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        existing_box_types = {bt.name: bt for bt in BoxType.objects.all()}
        ...
        storage_locations_by_code = {
            sl.location_code: sl
            for sl in StorageLocation.objects.all()
            if sl.location_code
        }
        ...
        for data in source_data:
            ...
            if not box_type_name:
                ...
                logger.info(
                    f"Box {legacy_id} has no box type specified. Using 'Unknown' box type."
                )
            ...
            if not box_type:
                logger.error(
                    "'Unknown' box type not found. Cannot proceed with import."
                )
                continue
            ...
            if storage_location_uuid:
                logger.debug(
                    f"Box {legacy_id} has direct storage location UUID: {storage_location_uuid}"
                )
            ...
            if not storage_location and data.get("data_"):
                try:
                    ...
                    if data_json.get("Stamm_Lagerort"):
                        ...
                        logger.debug(
                            f"Box {legacy_id} has JSON storage location UUID: {storage_location_uuid}"
                        )
                        storage_location = storage_locations_by_uuid.get(
                    ...
                    if not storage_location and data_json.get("Lagerort"):
                        ...
                        logger.debug(
                            f"Box {legacy_id} has JSON location code: {location_code}"
                        )
        ...
        logger.info(
            f"Boxes with storage location reference not found: {boxes_with_location_not_found}"
        )
...
class BoxSlotTransformer(BaseTransformer):
    ...
    def transform(self, source_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        existing_boxes = {str(box.legacy_id): box for box in Box.objects.all()}
...
class ProductInventoryTransformer(BaseTransformer):
    ...
    def transform(self, source_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Transformed {len(transformed_records)} product inventory records")
        return transformed_records
