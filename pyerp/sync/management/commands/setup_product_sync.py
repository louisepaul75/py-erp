...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        config_path = Path(settings.BASE_DIR) / "sync" / "config" / "products_sync.yaml"
        try:
        ...
        if created:
            self.stdout.write(f"Created sync mapping for parent products: {mapping}")
        else:
            self.stdout.write(f"Updated sync mapping for parent products: {mapping}")
        ...
        if "variants" in config:
            ...
            if created:
                self.stdout.write(f"Created sync source for variants: {variant_source}")
            else:
            ...
            if created:
                self.stdout.write(f"Created sync target for variants: {variant_target}")
            else:
                self.stdout.write(f"Updated sync target for variants: {variant_target}")
