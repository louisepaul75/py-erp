    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if sales_record_config:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully set up sales_records mapping (ID: {sales_record_mapping.id})")
                )
            else:
                self.stdout.write(
                    self.style.WARNING("sales_records configuration not found in YAML file")
                )
            ...
            if sales_record_item_config:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully set up sales_record_items mapping (ID: {sales_record_item_mapping.id})")
                )
            else:
                self.stdout.write(
                    self.style.WARNING("sales_record_items configuration not found in YAML file")
                )
            
            self.stdout.write(self.style.SUCCESS("Sales record sync mappings set up successfully"))
            
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"Error setting up sales record sync mappings: {str(e)}"))
    ...
    def _create_mapping(self, entity_type, config, force=False):
        ...
        try:
            ...
            try:
                ...
                if not force:
                    self.stdout.write(f"Mapping for {entity_type} already exists (ID: {existing_mapping.id})")
            ...
            if not transformer_config:
                raise CommandError(f"Transformer configuration not found for {entity_type}")
            ...
            target_defaults = {
                ...
                'config': {
                    ...
                    'parent_mapping': loader_config.get('config', {}).get('parent_mapping'),
                    'update_strategy': loader_config.get('config', {}).get('update_strategy'),
            ...
            mapping_config = {
                ...
                'transform_method': transformer_config.get('config', {}).get('transform_method'),
                ...
                'composite_key': transformer_config.get('config', {}).get('composite_key')
            }
