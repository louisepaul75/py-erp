...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if (
            record_result
            and "synced_ids" in record_result
            and record_result["synced_ids"]
        ):
            synced_legacy_ids = str(record_result["synced_ids"])
            self.stdout.write(
                f"Using {len(synced_legacy_ids)} synced legacy IDs for line items"
            )
        else:
            ...
            if not options.get("limit"):
                ...
                synced_legacy_ids = [
                    str(legacy_id)
                    for legacy_id in SalesRecord.objects.values_list(
                        "legacy_id", flat=True
                    )
                ]
                self.stdout.write(
                    f"Using all {len(synced_legacy_ids)} legacy IDs from database for line items"
                )
            else:
                self.stdout.write(
                    "No records were synced and a limit was specified, skipping line items sync"
                )

        if synced_legacy_ids:
            # Create filter query for line items
            line_items_params = self._build_query_params(
                options, component="sales_record_items"
            )
            ...
            self.stdout.write(
                f"Line items filter query: {line_items_params['filter_query']}"
            )

            # Then sync sales record items
            self.stdout.write("Syncing sales record items...")
            self._run_sync("sales_record_items", line_items_params, options)
        else:
    ...
    def _build_query_params(self, options, component=None):
        ...
        if component is None or component == "sales_records":
            ...
            if options["days"]:
                ...
                self.stdout.write(
                    f"Filtering records modified since {date_str} ({days} days ago)"
                )
            # Add default date filter for incremental sync (last 5 years)
            elif not options["full"] and not (
                "filter_query" in query_params
                and any(f[0] == "created_date" for f in query_params["filter_query"])
            ):
                ...
                self.stdout.write(
                    f"Adding default date filter: records from {date_str} (last 5 years)"
                )
        elif component == "sales_record_items":
            self.stdout.write(
                "Skipping date filters for sales_record_items - using only AbsNr filter"
            )
    ...
    def _get_or_create_mapping(self, mapping_name: str) -> SyncMapping:
        ...
        try:
            ...
            source_config["table_name"] = source_config.get("config", {}).get(
                "table_name"
            )
            ...
            source, _ = SyncSource.objects.get_or_create(
                name=source_config.get("type", "legacy_api"), defaults=source_defaults
            )
            if not _:
            ...
            target, _ = SyncTarget.objects.get_or_create(
                name=target_config.get("type", "django_model"), defaults=target_defaults
            )
            if not _:
    ...
    def _run_sync(self, component, query_params, options):
        ...
        try:
            ...
            transformer = SalesRecordTransformer(
                config={
                    "field_mappings": mapping.mapping_config.get("transformer", {})
                    .get("config", {})
                    .get("field_mappings", {}),
                    "validation_rules": mapping.mapping_config.get("transformer", {})
                    .get("config", {})
            ...
            loader = DjangoModelLoader(
                config={
                    "app_name": mapping.mapping_config.get("loader", {})
                    .get("config", {})
                    .get("app_name", "sales"),
                    "model_name": mapping.mapping_config.get("loader", {})
                    .get("config", {})
                    .get(
                        "model_name",
                        (
                            ...
                            else "SalesRecordItem"
                        ),
                    ),
                    "unique_field": mapping.mapping_config.get("loader", {})
                    .get("config", {})
                    .get("unique_field", "legacy_id"),
                    "update_strategy": mapping.mapping_config.get("loader", {})
                    .get("config", {})
                    .get("update_strategy", "update_or_create"),
                    "model_mapping": mapping.mapping_config.get("model_mapping", {}),
                    "field_mapping": mapping.mapping_config.get("field_mapping", {}),
                    "relation_mapping": mapping.mapping_config.get(
                        "relation_mapping", {}
                    ),
                }
            )
                ...
                for record in records:
                    ...
                    try:
                        ...
                        if component == "sales_records":
                            ...
                            if (
                                "AbsNr" in record
                                and "legacy_id" not in transformed_record
                            ):
            ...
            if component == "sales_record_items" and records:
                ...
                abs_nrs = [
                    record.get("AbsNr")
                    for record in records
                    if isinstance(record, dict) and record.get("AbsNr")
                ]
                ...
                if abs_nrs:
                    ...
                    if debug:
                        self.stdout.write(
                            f"Looking up sales records with legacy_ids: {unique_abs_nrs}"
                        )

                    # Get sales records by legacy_id
                    sales_records = SalesRecord.objects.filter(
                        legacy_id__in=unique_abs_nrs
                    )

                    if debug:
                        self.stdout.write(
                            f"Found {sales_records.count()} sales records in database"
                        )
                        for sr in sales_records:
                            self.stdout.write(
                                f"  - Sales record DB ID: {sr.id}, legacy_id: {sr.legacy_id}"
                            )

                    if not sales_records:
                        self.stdout.write(
                            f"No sales records found for legacy IDs: {unique_abs_nrs}"
                        )
                    else:
                        self.stdout.write(
                            f"Found {sales_records.count()} sales records for line items"
                        )
                        ...
                        for sales_record in sales_records:
                            ...
                            if debug:
                                self.stdout.write(
                                    f"Processing sales record ID: {sales_record.id}, legacy_id: {sales_record.legacy_id}"
                                )
                            ...
                            for item in records:
                                if (
                                    ...
                                    and str(item.get("AbsNr", "")) == legacy_id_str
                                ):
                                    record_items.append(item)

                            if debug:
                                self.stdout.write(
                                    f"Found {len(record_items)} line items matching legacy_id {legacy_id_str}"
                                )

                            if record_items:
                                self.stdout.write(
                                    f"Found {len(record_items)} line items for sales record {sales_record.legacy_id}"
                                )

                                # Add debug info for transformer
                                if debug:
                                    self.stdout.write(
                                        f"First record item: {record_items[0] if record_items else 'None'}"
                                    )
                                    self.stdout.write(
                                        f"First record item AbsNr: {record_items[0].get('AbsNr') if record_items else 'None'}"
                                    )

                                # Transform line items
                                if debug:
                                    self.stdout.write(
                                        f"Calling transformer.transform_line_items with {len(record_items)} items and sales_record.id={sales_record.id}"
                                    )
                                ...
                                for item in record_items:
                                    if (
                                        not item
                                        or "AbsNr" not in item
                                        or "PosNr" not in item
                                    ):
                                        if debug:
                                            self.stdout.write(
                                                f"Invalid line item data: {item}"
                                            )
                                        continue
                                    ...
                                    try:
                                        ...
                                        if product_code:
                                            try:
                                                ...
                                                VariantProduct.MultipleObjectsReturned
                                            ):
                                        ...
                                        line_subtotal = self._to_decimal(
                                            item.get("Pos_Betrag", 0)
                                        )
                                        ...
                                        if debug:
                                            self.stdout.write(
                                                f"Manually transformed item: {transformed_item}"
                                            )
                                    except Exception as e:
                                        if debug:
                                            self.stdout.write(
                                                f"Error manually transforming item: {str(e)}"
                                            )

                                if debug:
                                    self.stdout.write(
                                        f"Manually transformed items count: {len(transformed_items)}"
                                    )

                                if transformed_items:
                                    self.stdout.write(
                                        f"Transformed {len(transformed_items)} line items for sales record {sales_record.legacy_id}"
                                    )
                                    ...
                                    for item in transformed_items:
                                        try:
                                            ...
                                            if debug:
                                                self.stdout.write(
                                                    f"Saving line item: {item}"
                                                )

                                            # Load the item
                                            if debug:
                                                self.stdout.write(
                                                    f"Calling loader.load with item: {str(item)[:100]}..."
                                                )
                                            ...
                                            if debug:
                                                self.stdout.write(
                                                    f"Loader result: created={result.created}, updated={result.updated}, errors={result.errors}"
                                                )
                                                if result.errors > 0:
                                                    self.stdout.write(
    ...
    def _sanitize_record_data(self, record):
        ...
        for key, value in record.items():
            ...
            # Handle lists of dictionaries
                ...
                sanitized[key] = [
                    self._sanitize_record_data(item) if isinstance(item, dict) else item
                    for item in value
                ]
            else:
                sanitized[key] = value
    ...
    def _get_sync_mapping(self, mapping_name):
        ...
        try:
            ...
            self.stderr.write(
                self.style.ERROR(
                    f"Error getting sync mapping '{mapping_name}': {str(e)}"
                )
            )
            return None
