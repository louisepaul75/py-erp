...
class LoadResult:
    ...
    def add_error(
        ...
        self.error_details.append(
            {"record": record, "error": str(error), "context": context}
        )
        ...
        for record in records:
            try:
                ...
                loaded_record = self.load_record(
                    lookup_criteria, prepared_record, update_existing
                )
                ...
                result.add_error(
                    record=record, error=e, context={"update_existing": update_existing}
                )
                logger.error(f"Error loading record: {e}", extra={"record": record})
