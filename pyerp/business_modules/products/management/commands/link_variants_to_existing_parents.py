...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            with transaction.atomic():
                ...
                linked_variants, still_orphaned, errors = self._process_variants(
                    variants_without_parent,
                    dry_run,
                    linked_variants,
                    still_orphaned,
                    errors,
        ...
        self.stdout.write(
            f"Variants {'would be' if dry_run else ''} linked to existing parents: {linked_variants}",
        )
        ...
        self.stdout.write(self.style.SUCCESS("\n=== NEXT STEPS ==="))
        self.stdout.write(
            "1. For variants still without parents, consider creating placeholder parents",
        )
        self.stdout.write(
            "2. Run validation checks to ensure proper parent-child relationships",
        )
        self.stdout.write(self.style.SUCCESS("\n=== OPERATION COMPLETE ===\n"))
    ...
    def _process_variants(
        ...
        for variant in variants:
            try:
                matching_parent = ParentProduct.objects.filter(
                ...
                if matching_parent:
                    ...
                    self.stdout.write(
                        f"{'[DRY RUN] ' if dry_run else ''}Linked variant {variant.sku} to existing parent {matching_parent.sku}",
                    )
                    linked_variants += 1
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"{'[DRY RUN] ' if dry_run else ''}No matching parent found for variant {variant.sku} with Familie {variant.legacy_familie}",
                        ),
                    )
                    still_orphaned += 1
            except Exception as e:
                errors += 1
                error_msg = f"Error processing variant {variant.sku}: {e!s}"
