    ...
    if use_1password and not settings.EMAIL_HOST_PASSWORD:
        ...
        if username:
            ...
            password = get_email_password(
                email_username=username, item_name=item_name or None
            )
...
def send_test_email(to_email, subject=None, context=None):
    ...
    if not _ensure_password_from_1password():
        logger.error(
            "Cannot send test email: Failed to retrieve password from 1Password"
        )
        return False
    ...
    try:
        ...
        logger.info(
            f"Sending test email with settings: HOST={settings.EMAIL_HOST}, PORT={settings.EMAIL_PORT}, "
...
def send_html_email(
    ...
    try:
        ...
        msg = EmailMultiAlternatives(
            subject, text_content, from_email, to_email, cc=cc, bcc=bcc
        )
        msg.attach_alternative(html_content, "text/html")
