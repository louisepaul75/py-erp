...
class EmailLogAdmin(admin.ModelAdmin):
    list_display = ('subject', 'to_email_display', 'status', 'created_at', 'sent_at', 'opens', 'clicks')
    list_filter = ('status', 'created_at', 'sent_at', 'esp')
    search_fields = ('subject', 'to_email', 'from_email', 'message_id')
    readonly_fields = (
        'message_id', 'esp_message_id', 'created_at', 'updated_at', 
        'sent_at', 'delivered_at', 'opens', 'clicks', 'html_preview'
    )
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('message_id', 'subject', 'from_email', 'to_email', 'cc_email', 'bcc_email')
        }),
        (_('Content'), {
            'fields': ('body_text', 'html_preview')
        }),
        (_('Status'), {
            'fields': ('status', 'error_message')
        }),
        (_('ESP Information'), {
            'fields': ('esp', 'esp_message_id')
        }),
        (_('Tracking'), {
            'fields': ('opens', 'clicks')
        }),
...
class EmailEventAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('email_log', 'event_type', 'timestamp', 'data', 'user_agent', 'ip_address')
