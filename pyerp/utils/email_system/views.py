        ...
        try:
            ...
            smtp_settings = {
                ...
                "encryption": (
                    ...
                    else ("ssl" if settings.EMAIL_USE_SSL else "none")
                ),
                "use_1password": getattr(settings, "EMAIL_USE_1PASSWORD", False),
                "1password_item_name": getattr(
                    settings, "EMAIL_1PASSWORD_ITEM_NAME", ""
                ),
            }

            return JsonResponse({"success": True, "data": smtp_settings})
        ...
        try:
            ...
            if "development" in env_file:
                env_path = os.path.join(
                    settings.BASE_DIR.parent, "config", "env", ".env.dev"
                )
            elif "production" in env_file:
                env_path = os.path.join(
                    settings.BASE_DIR.parent, "config", "env", ".env.prod"
                )
            else:
                env_path = os.path.join(
                    settings.BASE_DIR.parent, "config", "env", ".env"
                )
            ...
            if use_1password:
                password = get_email_password(
                    email_username=data["username"], item_name=op_item_name or None
                )
                if password:
            ...
            logger.info(
                f"SMTP settings updated: {data['host']}:{data['port']} (user: {data['username']})"
            )

            return JsonResponse(
                {"success": True, "message": "SMTP settings updated successfully"}
            )
        except Exception as e:
    ...
    def post(self, request):
        ...
        try:
            ...
            if success:
                ...
                error_message = (
                    "Unknown error sending email. Check server logs for details."
                )
                if recent_log:
...
class EmailLogAPIView(APIView):
    ...
    def delete(self, request):
        ...
        try:
            ...
            return Response({"success": True, "message": f"Deleted {count} email logs"})
        except Exception as e:
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                return JsonResponse(
                    {"success": False, "error": "Port must be a number"}, status=400
                )
            ...
            if port == 465 and data["encryption"] != "ssl":
                logger.info(
                    f"Auto-correcting encryption from {data['encryption']} to 'ssl' for port 465"
                )
                data["encryption"] = "ssl"
            elif port == 587 and data["encryption"] != "tls":
                logger.info(
                    f"Auto-correcting encryption from {data['encryption']} to 'tls' for port 587"
                )
                data["encryption"] = "tls"
            ...
            env_file = os.environ.get(
                "DJANGO_ENV_FILE",
                os.path.join(settings.BASE_DIR, "config", "env", ".env.dev"),
            )
            ...
            logger.info(
                f"Email settings updated: {data['host']}:{data['port']} (user: {data['username']})"
            )
