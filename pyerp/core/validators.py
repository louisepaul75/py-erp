...
This module provides a flexible, extensible validation system for ensuring data integrity
throughout the application. It supports field-level validation, cross-field business rules,
...
class RequiredValidator(Validator):
    ...
    def __init__(self, error_message=None, message=None):
        ...
        Args:
            ...
            message: Alternative parameter name for error message (for backward compatibility)
        """
        # Allow either message or error_message parameter for flexibility
...
class DecimalValidator(Validator):
    ...
    def _validate(self, value, result, **kwargs):
        ...
        try:
            ...
            result.add_error(field_name, f"Value must be a valid decimal number")
            return
...
class SkuValidator(RegexValidator):
    ...
    def __init__(self, error_message=None):
        ...
        # SKU pattern allows letters, numbers, hyphens, and dots
        # Must start with a letter or number, not special characters
        pattern = r'^[A-Za-z0-9][A-Za-z0-9\-\.]*$'
        message = error_message or "SKU must be in a valid format (letters, numbers, hyphens, dots, no spaces)"
...
class CompoundValidator(Validator):
    ...
    def __init__(self, validators, require_all_valid=True, error_message=None):
        ...
        Args:
            ...
            require_all_valid: If True, all validators must pass; if False, at least one must pass
            error_message: Custom error message
        """
    ...
    def _validate(self, value, result, **kwargs):
        ...
        if self.require_all_valid:
            ...
            # Only add errors if all validators fail
            if all(not val_result.is_valid for val_result in validation_results):
                result.add_error(field_name, self.message)
                # Include specific errors from validators
                for val_result in validation_results:
...
class BusinessRuleValidator(Validator):
    ...
    def _validate(self, value, result, **kwargs):
        ...
        # The validation function should return True if valid, False if invalid
...
class ImportValidator:
    ...
    def validate_row(self, row_data, row_index=None):
        ...
        for field_name, value in row_data.items():
            # Check if there's a validation method for this field
            ...
            if validator_method:
                try:
                    ...
                    transformed_value, field_result = validator_method(
                        value, row_data, row_index=row_index
                    )
                    
                    # Handle validation result
                    if field_result.is_valid:
                        # If valid, use transformed value
                ...
                # No validator for this field, pass through unchanged
                validated_data[field_name] = value
    ...
    def _post_validate_row(self, validated_data, result, row_index=None):
        ...
        Override this method to implement business rules that span multiple fields.
        ...
        """
        # Default implementation calls cross_validate_row if it exists
...
def validate_model_data(instance):
    ...
    """
    # This is typically called from a model's clean() method
    errors = {}
