...
class Command(BaseCommand):
    ...
    def extract_messages(self, locale=None):
        ...
        try:
            ...
            if result.returncode != 0:
                self.stderr.write(
                    self.style.ERROR(f"Error extracting messages: {result.stderr}"),
                )
                return
    ...
    def compile_messages(self, locale=None):
        ...
        try:
            ...
            if result.returncode == 0:
                self.stdout.write(
                    self.style.SUCCESS("Messages compiled successfully using Django."),
                )
                return
            self.stdout.write(
                self.style.WARNING(
                    "Django compilemessages failed, falling back to Python implementation.",
                ),
            )
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Django compilemessages error: {e}. Falling back to Python implementation.",
                ),
            )
    ...
    def _compile_messages_python(self, locale=None):
        ...
        if not POLIB_AVAILABLE:
            self.stdout.write(
                self.style.WARNING(
                    "polib is not installed. Please run 'pip install polib' for better .mo file generation.",
                ),
            )
            return
        ...
        for locale_code, locale_path in locale_dirs:
            ...
            if os.path.exists(po_path):
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Compiled {locale_code} messages to {mo_path}",
                        ),
                    )
                except Exception as e:
                    self.stderr.write(
                        self.style.ERROR(f"Error compiling {po_path}: {e}"),
                    )
            else:
                self.stderr.write(
                    self.style.WARNING(
                        f"Locale {locale_code}: .po file not found at {po_path}",
                    ),
                )
        ...
        for locale_code, locale_path in locale_dirs:
            ...
            if os.path.exists(po_path):
                ...
                try:
                    ...
                    self.stderr.write(
                        self.style.ERROR(f"  Error analyzing {po_path}: {e}"),
                    )
            else:
                self.stderr.write(
                    self.style.WARNING(
