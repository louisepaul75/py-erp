"""
Management command to migrate data from the old Product model to the new ParentProduct and VariantProduct models.
"""
...
class Command(BaseCommand):
    ...
    def migrate_parent_products(self, limit=None, skip_existing=False, dry_run=False):
        """
        Migrate parent products from the old Product model to the new ParentProduct model.
        """
        ...
        for old_parent in parent_products:
            try:
                # Skip if the parent already exists and skip_existing is True
                if skip_existing and ParentProduct.objects.filter(legacy_id=old_parent.legacy_id).exists():
                    self.stdout.write(f'Skipping existing parent product: {old_parent.sku}')
                    stats['skipped'] += 1
                    continue
                ...
                if dry_run:
                    self.stdout.write(f'Would create parent product: {parent_data["sku"]} - {parent_data["name"]}')
                    stats['created'] += 1
                    # Add to mapping for dry run
                ...
                self.stdout.write(
                    self.style.ERROR(f'Error migrating parent product {old_parent.sku}: {str(e)}')
                )
                stats['errors'] += 1
    ...
    def migrate_variant_products(self, limit=None, skip_existing=False, dry_run=False):
        """
        Migrate variant products from the old Product model to the new VariantProduct model.
        """
        ...
        self.stdout.write(f'Found {variant_products.count()} variant products to migrate')
        ...
        for old_variant in variant_products:
            try:
                ...
                if skip_existing and VariantProduct.objects.filter(legacy_id=old_variant.legacy_id).exists():
                    self.stdout.write(f'Skipping existing variant product: {old_variant.sku}')
                    stats['skipped'] += 1
                    continue
                ...
                if old_variant.parent_id:
                    try:
                        # Find the new parent product
                        parent = ParentProduct.objects.get(legacy_id=old_variant.parent.legacy_id)
                    except ParentProduct.DoesNotExist:
                        # Try to find by SKU
                        try:
                            parent = ParentProduct.objects.get(sku=old_variant.parent.sku)
                        except ParentProduct.DoesNotExist:
                            pass
                ...
                if not parent and old_variant.base_sku:
                    try:
                        parent = ParentProduct.objects.get(base_sku=old_variant.base_sku)
                    except ParentProduct.DoesNotExist:
                        pass
                
                if not parent:
                    self.stdout.write(
                        self.style.WARNING(f'Parent product not found for variant: {old_variant.sku}')
                    )
                ...
                if dry_run:
                    self.stdout.write(f'Would create variant product: {variant_data["sku"]} - {variant_data["name"]}')
                    stats['created'] += 1
                    continue
                ...
                self.stdout.write(
                    self.style.ERROR(f'Error migrating variant product {old_variant.sku}: {str(e)}')
                )
                stats['errors'] += 1
