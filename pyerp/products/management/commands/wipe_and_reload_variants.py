"""
Management command to wipe all variant products and reload them from Artikel_Variante.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE('Starting variant product wipe and reload from Artikel_Variante...'))
        
        # Get current count of variant products
        initial_count = VariantProduct.objects.count()
        self.stdout.write(self.style.SUCCESS(f'Current variant product count: {initial_count}'))
        ...
        if not preserve_existing and not dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(f'Deleted {deleted_count} variant products'))
        elif preserve_existing:
            self.stdout.write(self.style.WARNING('Preserving existing variant products...'))
        ...
        self.stdout.write(self.style.NOTICE('Fetching Artikel_Variante data from legacy ERP...'))
        try:
            # Use wsz_api to fetch the data
            artikel_variante_df = fetch_data_from_api('Artikel_Variante', top=10000, new_data_only=False)
            ...
            self.stdout.write(self.style.SUCCESS(f'Fetched {total_records} records from Artikel_Variante'))
            ...
            familie_to_parent = {}
            for _, parent_product in ParentProduct.objects.all().values_list('id', 'legacy_id'):
                if parent_product:
                    familie_to_parent[parent_product] = parent_product
            
            self.stdout.write(f'Loaded {len(familie_to_parent)} parent products for Familie_ mapping')
            ...
            batch_count = (len(records) + batch_size - 1) // batch_size  # ceiling division
            ...
            for batch_index in range(batch_count):
                ...
                self.stdout.write(self.style.NOTICE(f'Processing batch {batch_index + 1}/{batch_count} (records {batch_start + start_from} to {batch_end + start_from - 1})'))
                ...
                with transaction.atomic():
                    for i, row in enumerate(batch):
                        ...
                        try:
                            # Get essential data
                            variant_key = row['__KEY']  # Primary key in the source database
                            variant_uid = row.get('UID', None)  # UID field from Artikel_Variante
                            product_name = row['Bezeichnung'] if pd.notna(row['Bezeichnung']) else "Unnamed Variant"
                            
                            # Get the Nummer field and ensure it's a string
                            nummer = row['Nummer'] if pd.notna(row['Nummer']) else None
                            alte_nummer = row['alteNummer'] if pd.notna(row['alteNummer']) else None
                            ...
                            if nummer is not None:
                                ...
                                # Skip records without a Nummer or alteNummer as we need one for the SKU
                                self.stdout.write(self.style.WARNING(f"Skipping record {record_index}: No Nummer or alteNummer field"))
                                stats['errors'] += 1
                                continue
                            
                            is_active = bool(row['aktiv']) if pd.notna(row.get('aktiv')) else True
                            ...
                            if familie_id in familie_to_parent:
                                try:
                                    parent = ParentProduct.objects.get(legacy_id=familie_id)
                            ...
                            if parent is None and base_sku:
                                try:
                                    parent = ParentProduct.objects.filter(base_sku=base_sku).first()
                                except Exception:
                                    pass
                            
                            if parent is None:
                                if verbose:
                                    self.stdout.write(self.style.WARNING(f"No parent product found for variant {record_index}: {sku}"))
                            ...
                            if preserve_existing:
                                try:
                                    ...
                                    existing_variant = VariantProduct.objects.get(legacy_uid=variant_uid)
                                    exists = True
                                except VariantProduct.DoesNotExist:
                                    # Then try by SKU
                                    try:
                                        existing_variant = VariantProduct.objects.get(sku=sku)
                                        exists = True
                                    except VariantProduct.DoesNotExist:
                                        # Then try by legacy ID
                                        try:
                                            existing_variant = VariantProduct.objects.get(legacy_id=variant_key)
                                            exists = True
                                        except VariantProduct.DoesNotExist:
                                            existing_variant = VariantProduct()
                                            exists = False
                            else:
                                # If we wiped all variants, we're creating new ones
                                existing_variant = VariantProduct()
                                exists = False
                            ...
                            existing_variant.legacy_id = variant_key  # Use KEY as legacy_id
                            existing_variant.legacy_uid = variant_uid  # Store UID separately
                            
                            # Store original field names for display purposes
                            existing_variant.legacy_key = variant_key      # Original __KEY field
                            existing_variant.legacy_uid_original = variant_uid  # Original UID field
                            existing_variant.legacy_familie = familie_id  # Original Familie_ field
                            ...
                            # Debug info
                            if verbose:
                                self.stdout.write(f"Record {record_index}: KEY={variant_key}, UID={variant_uid}, Familie_={familie_id}")
                            
                            # Save the product if not a dry run
                            if not dry_run:
                                existing_variant.save()
                                if verbose:
                                    self.stdout.write(f"Processed record {record_index}: {sku} - {product_name}")
                            ...
                            stats['errors'] += 1
                            self.stdout.write(self.style.ERROR(f"Error processing record {record_index}: {str(e)}"))
                    ...
                    if dry_run:
                        self.stdout.write(self.style.WARNING("DRY RUN - No changes were saved for this batch"))
                        transaction.set_rollback(True)
                
                # Report batch progress
                self.stdout.write(self.style.SUCCESS(f'Completed batch {batch_index + 1}/{batch_count}'))
                self.stdout.write(f'Progress: Created: {stats["created"]}, Updated: {stats["updated"]}, Errors: {stats["errors"]}')
        
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error fetching or processing data: {str(e)}"))
            return
        
        # Transaction completed, get final count
        final_count = VariantProduct.objects.count()
        self.stdout.write(self.style.SUCCESS(f'Final variant product count: {final_count}'))
        ...
        self.stdout.write("\nNext steps:")
        self.stdout.write("1. Run 'python manage.py fix_variant_parent_relationships' to update any missing variant-parent relationships")
