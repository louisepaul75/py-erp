"""
Management command to fix missing variants by creating placeholder parent products.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        migrated_variants = set(VariantProduct.objects.values_list('legacy_id', flat=True))
        ...
        for i, variant in enumerate(missing_variants[:20]):  # Show first 20 for brevity
        ...
        if dry_run and create_parents:
            ...
            self.stdout.write(self.style.SUCCESS('Placeholder parents created successfully.'))
    ...
    def create_placeholder_parents(self, prefix_groups, group_by_prefix):
        ...
        with transaction.atomic():
            if group_by_prefix:
                # Create one parent per prefix group
                for prefix, variants in prefix_groups.items():
                    ...
                    if created:
                        created_parents += 1
                        self.stdout.write(f"  Created parent: {parent_sku} - {parent_name}")
                    
                    # Create variants
                    for variant in variants:
                        # Extract variant code from SKU
                ...
                # Create one parent per variant
                for prefix, variants in prefix_groups.items():
                    for variant in variants:
                        # Create parent with same base SKU
        ...
        self.stdout.write(self.style.SUCCESS(f"Created {created_parents} placeholder parents and migrated {migrated_variants} variants"))
    ...
    def simulate_parent_creation(self, prefix_groups, group_by_prefix):
        ...
        if group_by_prefix:
            # Simulate one parent per prefix group
            for prefix, variants in prefix_groups.items():
                parent_sku = f"{prefix}000"
                parent_name = f"Product Group {prefix}xxx"
                self.stdout.write(f"  Would create parent: {parent_sku} - {parent_name} for {len(variants)} variants")
        else:
            # Simulate one parent per variant
            variant_count = sum(len(variants) for variants in prefix_groups.values())
            self.stdout.write(f"  Would create {variant_count} individual parent products, one for each variant") 