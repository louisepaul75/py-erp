...
class Command(BaseCommand):
    help = 'Import parent products from the Art_Kalkulation table in the legacy 4D system'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Starting parent product import from Art_Kalkulation table...")
        ...
        try:
            ...
            if df is None or len(df) == 0:
                raise CommandError("No data returned from Art_Kalkulation table")
    ...
    def process_parent_products(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Error processing record {idx}: {str(e)}"))
                if self.debug:
                    import traceback
                    self.stdout.write(traceback.format_exc())
                skipped += 1
        ...
        # Print summary
        self.stdout.write(self.style.SUCCESS(f"Processed {total} parent product records:"))
    ...
    def process_parent_product(self, row):
        ...
        if not parent_sku:
            if self.debug:
                self.stdout.write(f"  Skipping row - No valid SKU")
            return 'skipped'
        ...
        if variant_count == 0 and not self.update and not self.force:
            if self.debug:
                self.stdout.write(f"  Skipping orphaned parent {parent_sku} - No variants exist")
            return 'orphaned'
        ...
        if existing_parent:
            if not self.update:
                if self.debug:
                    self.stdout.write(f"  Skipping existing parent {parent_sku} (update not enabled)")
                return 'skipped'
