"""
Management command to wipe all parent products and reload them from Artikel_Familie.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE('Starting parent product wipe and reload from Artikel_Familie...'))
        
        # Get current count of parent products
        initial_count = ParentProduct.objects.count()
        self.stdout.write(self.style.SUCCESS(f'Current parent product count: {initial_count}'))
        ...
        if not preserve_relationships and not dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(f'Deleted {deleted_count} parent products'))
        elif preserve_relationships:
            self.stdout.write(self.style.WARNING('Preserving parent products for relationships...'))
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f'Fetched {total_records} records from Artikel_Familie'))
            ...
            for batch_index in range(batch_count):
                ...
                with transaction.atomic():
                    for i, row in enumerate(batch):
                        ...
                        try:
                            # Get essential data
                            familie_key = row['__KEY']  # Primary key in the source database
                            familie_uid = row.get('UID', None)  # UID field from Artikel_Familie
                            ...
                            if nummer is not None:
                                ...
                                # Skip records without a Nummer as we need this for the SKU
                                self.stdout.write(self.style.WARNING(f"Skipping record {record_index}: No Nummer field"))
                                stats['errors'] += 1
                                continue
                            ...
                            if preserve_relationships:
                                try:
                                    ...
                                    try:
                                        parent_product = ParentProduct.objects.get(legacy_id=familie_uid)
                                        exists = True
                                    except ParentProduct.DoesNotExist:
                                        try:
                                            parent_product = ParentProduct.objects.get(legacy_id=familie_key)
                                            exists = True
                                        except ParentProduct.DoesNotExist:
                                            parent_product = ParentProduct()
                                            exists = False
                            else:
                                # If we wiped all parents, we're creating new ones
                                parent_product = ParentProduct()
                                exists = False
                            ...
                            parent_product.legacy_id = familie_uid if familie_uid else familie_key  # Prefer UID, fallback to KEY
                            parent_product.legacy_uid = familie_uid  # Store the UID separately
                            parent_product.__KEY = familie_key  # Store the original key
                            parent_product.UID = familie_uid  # Store the original UID
        ...
        self.stdout.write(self.style.SUCCESS(f'Final parent product count: {final_count}'))
