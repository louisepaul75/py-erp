...
class Command(BaseCommand):
    help = (
        "Fix variant-parent relationships using Familie_ column from Artikel_Variante"
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                "Starting variant-parent relationship fix using Familie_ column...",
            ),
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Found {total_variants} variants to check"),
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Fetched {len(artikel_variante_df)} records from Artikel_Variante",
                ),
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Fetched {len(artikel_familie_df)} records from Artikel_Familie",
                ),
            )
            ...
            if len(artikel_familie_df) > 0:
                ...
                self.stdout.write(
                    f"Sample columns in Artikel_Familie: {list(sample_row.keys())}",
                )
                ...
                for idx, row in artikel_familie_df.head(10).iterrows():
                    if "Nummer" in row and pd.notna(row["Nummer"]):
                        self.stdout.write(
                            f"  - UID: {row.get('UID', 'N/A')}, Nummer: {row['Nummer']}, Type: {type(row['Nummer'])}",
                        )
            ...
            self.stdout.write(
                f"Sample Familie UID values from Artikel_Familie: {sample_familie_ids_uid}",
            )
            self.stdout.write(
                f"Sample Familie KEY_ values from Artikel_Familie: {sample_familie_ids_key}",
            )

            # Debug: Print sample Familie_ values from variants
            sample_variant_familie_ids = list(
                set(list(legacy_id_to_familie.values())[:5]),
            )
            self.stdout.write(
                f"Sample Familie_ values from variants: {sample_variant_familie_ids}",
            )

            # Verify if Familie_ from Artikel_Variante matches any of the ID fields from Artikel_Familie
            self.stdout.write(
                "Checking if variant Familie_ values match any Familie IDs...",
            )
            for familie_value in sample_variant_familie_ids:
                if familie_value in familie_id_to_parent:
                    self.stdout.write(
                        f"  - Match found for Familie_ value: {familie_value}",
                    )
                else:
                    self.stdout.write(
                        f"  - No match found for Familie_ value: {familie_value}",
                    )

            # Debug: Print total parents found
            self.stdout.write(
                f"Total parent products mapped: {len(familie_id_to_parent)}",
            )

            # Debug: Sample mapping
            sample_mapping = dict(list(familie_id_to_parent.items())[:5])
            self.stdout.write(
                f"Sample family ID to parent mapping: {[(k, v.sku) for k, v in sample_mapping.items()]}",
            )
            ...
            with transaction.atomic():
                for variant in variants:
                    try:
                        ...
                        self.stdout.write(
                            f"Checking variant: ID={variant.id}, SKU={variant.sku}, Legacy ID={variant.legacy_id}, Current Parent ID={parent_id}",
                        )
                        ...
                        if variant.legacy_id in legacy_id_to_familie:
                            ...
                            self.stdout.write(
                                f"Found Familie ID {familie_id} by legacy_id",
                            )
                            ...
                            variant.legacy_sku and variant.legacy_sku in sku_to_familie
                        ):
                            familie_id = sku_to_familie[variant.legacy_sku]
                            self.stdout.write(
                                f"Found Familie ID {familie_id} by legacy_sku",
                            )

                        if familie_id and familie_id in familie_id_to_parent:
                            parent = familie_id_to_parent[familie_id]
                            self.stdout.write(
                                f"Found parent product {parent.sku} by Familie ID {familie_id}",
                            )

                        if parent:
                            if variant.parent and variant.parent.id == parent.id:
                                self.stdout.write(
                                    f"Relationship already correct for variant {variant.id}",
                                )
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"Error processing variant {variant.id}: {e!s}",
                            ),
                        )
                        stats["errors"] += 1
                ...
                if dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS("Changes committed to the database"),
                    )
            ...
            self.stdout.write(self.style.ERROR(f"Error processing variants: {e!s}"))
            raise
