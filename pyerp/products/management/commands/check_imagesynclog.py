...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("Checking ImageSyncLog model structure..."),
        )
        ...
        for field in ImageSyncLog._meta.fields:
            ...
            if hasattr(field, "default"):
                ...
                default_str = "callable" if callable(default) else str(default)
            else:
                default_str = "None"
            ...
            self.stdout.write(
                f"{field.name:<20} {field_type:<20} Default: {default_str:<20}",
            )

            if field_type == "DateTimeField":
                self.stdout.write(f"{'  auto_now:':<20} {auto_now!s:<20}")
                self.stdout.write(f"{'  auto_now_add:':<20} {auto_now_add!s:<20}")
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write(
                f"{'Column Name':<20} {'Data Type':<20} {'Max Length':<15} {'Nullable':<10} {'Default':<30}",
            )
            self.stdout.write("-" * 100)
            ...
            for col in columns:
                ...
                self.stdout.write(
                    f"{col_name:<20} {data_type:<20} {max_length_str:<15} {nullable:<10} {default or ''!s:<30}",
                )
            ...
            if constraints:
                ...
                self.stdout.write("=" * 80)
                self.stdout.write(
                    f"{'Constraint Name':<40} {'Type':<20} {'Column':<20}",
                )
                self.stdout.write("-" * 80)
            ...
            cursor.execute(
                "SELECT pg_get_serial_sequence('products_imagesynclog', 'id')",
            )
        ...
        status_field = next(
            (f for f in ImageSyncLog._meta.fields if f.name == "status"),
            None,
        )
