...
class Command(BaseCommand):
    help = 'Checks the structure of the ImageSyncLog model and table'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Checking ImageSyncLog model structure...'))
        ...
        for field in ImageSyncLog._meta.fields:
            ...
            self.stdout.write(f"{field.name:<20} {field_type:<20} Default: {default_str:<20}")
            
            if field_type == 'DateTimeField':
                self.stdout.write(f"{'  auto_now:':<20} {str(auto_now):<20}")
                self.stdout.write(f"{'  auto_now_add:':<20} {str(auto_now_add):<20}")
            
            if field_type == 'CharField':
                self.stdout.write(f"{'  max_length:':<20} {max_length_str:<20}")
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                WHERE 
                    table_name = 'products_imagesynclog' 
                ORDER BY 
                    ordinal_position
            """)
            ...
            self.stdout.write(f"{'Column Name':<20} {'Data Type':<20} {'Max Length':<15} {'Nullable':<10} {'Default':<30}")
            self.stdout.write('-' * 100)
            
            for col in columns:
                col_name, data_type, max_length, nullable, default = col
                max_length_str = str(max_length) if max_length is not None else ""
                self.stdout.write(f"{col_name:<20} {data_type:<20} {max_length_str:<15} {nullable:<10} {str(default or ''):<30}")
            ...
            if constraints:
                self.stdout.write('\nTable Constraints:')
                self.stdout.write('=' * 80)
                self.stdout.write(f"{'Constraint Name':<40} {'Type':<20} {'Column':<20}")
                self.stdout.write('-' * 80)
            ...
            cursor.execute("SELECT pg_get_serial_sequence('products_imagesynclog', 'id')")
            sequence = cursor.fetchone()
            
            if sequence and sequence[0]:
                self.stdout.write(f'\nID Sequence: {sequence[0]}')
                cursor.execute(f"SELECT last_value, is_called FROM {sequence[0]}")
        ...
        if not missing_in_db and not missing_in_model:
            self.stdout.write(self.style.SUCCESS('\nAll fields match between model and database!'))
        
        # Validate status field
        status_field = next((f for f in ImageSyncLog._meta.fields if f.name == 'status'), None)
        ...
        if status_field and status_column:
            ...
            if model_max_length != db_max_length and db_max_length is not None:
                self.stdout.write(self.style.WARNING(
                    f'\nStatus field max_length mismatch: Model={model_max_length}, DB={db_max_length}'
                ))
            else:
                self.stdout.write(self.style.SUCCESS('\nStatus field max_length is consistent.'))
        
        self.stdout.write(self.style.SUCCESS('\nImageSyncLog structure check completed.')) 