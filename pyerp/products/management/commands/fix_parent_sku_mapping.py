...
class Command(BaseCommand):
    help = (
        "Fix SKU and legacy_id fields for parent products based on Artikel_Familie data"
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                "Starting parent product SKU mapping fix from Artikel_Familie...",
            ),
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Found {total_parents} parent products to check"),
        )
        ...
        try:
            ...
            for index, row in artikel_familie_df.iterrows():
                id_val = row["__KEY"]
                nummer_val = (
                    str(int(row["Nummer"])) if pd.notna(row.get("Nummer")) else None
                )
                oldart_val = (
                    str(row["oldArtKalk"]) if pd.notna(row.get("oldArtKalk")) else None
                )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"Error fetching data from Artikel_Familie API: {e!s}"
                ),
            )
            return
        ...
        for i in range(0, total_parents, batch_size):
            ...
            self.stdout.write(
                self.style.NOTICE(
                    f"Processing batch {i // batch_size + 1} of {(total_parents + batch_size - 1) // batch_size}",
                ),
            )
            ...
            with transaction.atomic():
                for parent in batch:
                    try:
                        self.stdout.write(
                            f"Checking parent: ID={parent.id}, SKU={parent.sku}, Legacy ID={parent.legacy_id}",
                        )
                        ...
                        # Strategy 3: Try to match by legacy_id against Nummer (handling swapped fields)
                            ...
                            parent.legacy_id
                            and parent.legacy_id in legacy_data_by_nummer
                        ):
                        ...
                        # Strategy 6: Try to match by legacy_id against oldArtKalk
                            ...
                            parent.legacy_id
                            and parent.legacy_id in legacy_data_by_oldart
                        ):
                        ...
                        if legacy_record:
                            ...
                            new_legacy_id = (
                                legacy_record["oldArtKalk"] or legacy_record["id"]
                            )

                            # Check if update is needed
                            if (
                                parent.sku != new_sku
                                or parent.legacy_id != new_legacy_id
                            ):
                                old_sku = parent.sku
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"Error processing parent ID={parent.id}: {e!s}",
                            ),
                        )
                        stats["errors"] += 1
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(
                        "DRY RUN - No changes were saved to the database for this batch",
                    ),
                )
