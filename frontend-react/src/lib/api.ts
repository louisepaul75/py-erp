...
export async function fetchOrders(): Promise<Order[]> {
  try {
    ...
    const orders: Order[] = await Promise.all(salesRecords.map(async (record) => {
      const itemsResponse = await instance.get(`sales/records/${record.id}/items/`).json<{results: any[]}>();
      ...
      const binLocationsResponse = await instance.get(`inventory/bin-locations/by-order/${record.id}/`).json<{results: BinLocation[]}>();
