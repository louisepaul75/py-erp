...
def analyze_coverage():
    ...
    for filename, file_data in data.get('files', {}).items():
        ...
        # Calculate metrics
        total_statements = len(file_data.get('executed_lines', [])) + len(file_data.get('missing_lines', []))
    ...
    for i, module in enumerate(modules[:15]):
        ...
        print(f"   Coverage: {module['coverage']:.1f}% ({module['executed_statements']}/{module['total_statements']} statements)")
        print()
    ...
    for dir_name, dir_modules in dir_coverage.items():
        ...
        print(f"{dir_name}: {dir_percentage:.1f}% ({executed_stmts}/{total_stmts} statements)")
