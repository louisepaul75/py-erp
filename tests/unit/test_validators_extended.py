...
This module contains additional tests for the validator classes in pyerp/core/validators.py
...
class TestValidationResult:
    ...
    def test_add_error(self):
        ...
        assert result.errors["field1"] == ["Error message 1", "Error message 3"]
    ...
    def test_add_warning(self):
        ...
        assert result.warnings["field1"] == ["Warning message 1", "Warning message 2"]
...
class TestRequiredValidator:
    ...
    def test_required_validator_without_value(self):
        ...
        # Note: Whitespace-only strings might be considered valid depending on implementation
        # This test should be adjusted based on the actual implementation
        # If the implementation doesn't consider whitespace as empty, this test should be omitted
        # Or the assertion should be changed to match the expected behavior
...
class TestRangeValidator:
    ...
    def test_range_validator_below_min(self):
        ...
        validator = RangeValidator(min_value=0, max_value=100, error_message="Value must be between 0 and 100")
        result = validator(-10)
        assert result.is_valid is False
        assert "field" in result.errors
...
class TestChoiceValidator:
    ...
    def test_choice_validator_invalid_choice(self):
        ...
        choices = ["red", "green", "blue"]
        validator = ChoiceValidator(choices, error_message="Invalid color choice")
...
class TestCompoundValidator:
    ...
    def test_compound_validator_all_invalid(self):
        ...
        assert result.is_valid is False
        assert "field" in result.errors
        assert len(result.errors["field"]) >= 1  # Should have at least one error message
    ...
    def test_compound_validator_not_require_all(self):
        ...
        assert result.is_valid is True  # Still valid because we don't require all
        
        result = validator("hello")  # Valid length, invalid regex
        assert result.is_valid is True  # Still valid because we don't require all
        
        result = validator("h")  # Invalid length, invalid regex
        assert result.is_valid is False  # Invalid because all validators failed
...
def test_validate_data():
    ...
    validators = [BusinessRuleValidator(context_validator, error_message="Admin only")]
