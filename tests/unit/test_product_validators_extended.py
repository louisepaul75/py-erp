...
with patch('django.utils.translation.gettext_lazy', lambda x: x):
    ...
    class TestProductImportValidator:
        ...
        def test_validate_sku_duplicate(self, validator):
            ...
            # Assert
            assert value == "ABC123"
            assert result.is_valid  # Still valid because duplicate is just a warning
        ...
        def test_validate_category_not_found(self, validator):
            ...
            # Setup
            MockProductCategory.objects.get.side_effect = MockProductCategory.DoesNotExist
        ...
        def test_post_validate_row(self, validator):
            ...
            row_data = {
                "sku": "ABC123",
                "is_parent": True,
                "variant_code": "V1"  # This should cause an error for a parent product
            }
            result = MagicMock()
