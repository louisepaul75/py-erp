...
class TestUserProfileView:
    ...
    def test_update_user_profile(self, request_factory, mock_user):
        ...
        request = request_factory.patch('/api/user/profile/', data=json.dumps(update_data), content_type='application/json')
        request.user = mock_user
...
class TestSystemSettingsView:
    ...
    def test_get_system_settings_non_staff(self, request_factory, mock_user):
        ...
        assert response.data['error'] == 'You do not have permission to view system settings'
    ...
    def test_update_system_settings_superuser(self, request_factory, mock_user):
        ...
        request = request_factory.patch('/api/system/settings/', data=json.dumps(update_data), content_type='application/json')
        request.user = mock_user
...
def test_db_error_view(request_factory):
    ...
    pyerp_core_views.test_db_error.side_effect = OperationalError("Simulated database error")
