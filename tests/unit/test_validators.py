...
class TestImportValidator:
    ...
    def test_validate_row_valid(self):
        ...
        row_data = {
            ...
            'age': '25',  # String, should be converted to in
            'is_vip': True
        }
        ...
        assert validated_data['age'] == 25  # Converted to in
...
class TestCompoundValidator:
    ...
    def test_some_invalid_require_all(self):
        ...
        result = validator("test", field_name="name")  # Too shor
        assert not result.is_valid
        assert "name" in result.errors
