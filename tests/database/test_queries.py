...
class TestProductQueries:
    ...
    def sample_products(self):
        ...
        parent1 = ParentProduct.objects.create(
            sku="PARENT001", name="Parent 1", base_sku="BASE001"
        )
        parent2 = ParentProduct.objects.create(
            sku="PARENT002", name="Parent 2", base_sku="BASE002"
        )

        # Create variants for parent1
        for i in range(3):
            VariantProduct.objects.create(
                sku=f"VAR00{i+1}", name=f"Variant {i+1}", parent=parent1
            )

        # Create variants for parent2
        for i in range(2):
            VariantProduct.objects.create(
                sku=f"VAR10{i+1}", name=f"Variant {i+1}", parent=parent2
            )
    ...
    def test_parent_with_variants_count(self, sample_products):
        ...
        variants_per_parent = (
            ParentProduct.objects.annotate(variant_count=Count("variants"))
    ...
    def test_complex_product_query(self, sample_products):
        ...
        parents_with_many_variants = ParentProduct.objects.annotate(
            variant_count=Count("variants")
        ).filter(variant_count__gt=2)

        complex_query = (
            VariantProduct.objects.filter(parent__in=parents_with_many_variants)
            .distinct()
            .order_by("sku")
        )
