...
class TestDatabaseConnectionMiddleware(TestCase):
    ...
    def setUp(self):
        ...
        self.middleware = DatabaseConnectionMiddleware(self._get_response)
...
class TestAuthExemptMiddleware(TestCase):
    ...
    def setUp(self):
        """Set up test data."""
        self.factory = RequestFactory()
        self.middleware = AuthExemptMiddleware(get_response=None)
    ...
    def test_auth_exempt_response_handling(self):
        ...
        response = self.middleware.process_response(request, redirect_response)
    ...
    def test_non_auth_response_handling(self):
        ...
        original_response = HttpResponse("Test response")
        processed_response = self.middleware.process_response(
            request, original_response
        )
        
        # Verify response wasn't changed
        self.assertEqual(processed_response, original_response)

    def test_auth_exempt_with_custom_response(self):
        """Test handling when _auth_exempt_response is set."""
        request = self.factory.get('/health/')
        request._auth_exempt = True
        custom_response = JsonResponse({
            'status': 'custom_response',
            'message': 'Custom health response'
        })
        ...
        response_data = response.json()
        self.assertEqual(response_data['status'], 'custom_response')
        self.assertEqual(response_data['message'], 'Custom health response') 