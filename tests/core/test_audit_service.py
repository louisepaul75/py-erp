...
class TestAuditService(TestCase):
    ...
    def test_log_event_basic(self):
        ...
        log_entry = AuditService.log_event(
            event_type=event_type, message=message, user=self.user
        )
    ...
    def test_log_event_with_request(self):
        ...
        log_entry = AuditService.log_event(
            event_type=event_type, message=message, request=self.sample_request
        )
    ...
    def test_log_event_with_related_object(self):
        ...
        related_user = User.objects.create_user(
            username="related_user", email="related@example.com", password="password123"
        )
        ...
        log_entry = AuditService.log_event(
            event_type=event_type, message=message, user=self.user, obj=related_user
        )
    ...
    def test_log_event_with_additional_data(self):
        ...
        self.assertEqual(
            log_entry.additional_data["changed_fields"], ["name", "description"]
        )
        self.assertEqual(log_entry.additional_data["old_values"]["name"], "Old Name")
    ...
    def test_log_logout(self, mock_log_event):
        ...
        mock_log_event.assert_called_with(
            AuditLog.EventType.LOGOUT, "User logged out", self.user, self.sample_request
        )
    ...
    def test_log_user_updated(self, mock_log_event):
        ...
        updated_user = User.objects.create_user(
            username="updateduser", email="updated@example.com", password="password123"
        )
        changed_fields = ["email", "first_name"]

        AuditService.log_user_updated(
            updated_user, self.user, self.sample_request, changed_fields
        )
        ...
        mock_log_event.reset_mock()
        AuditService.log_user_updated(updated_user, self.user, self.sample_request)
    ...
    def test_log_permission_change(self, mock_log_event):
        ...
        AuditService.log_permission_change(
            self.user, target_user, self.sample_request, added=added, removed=removed
        )
    ...
    def test_log_data_access(self, mock_log_event):
        ...
        AuditService.log_data_access(
            self.user, accessed_user, self.sample_request, action="viewed"
        )
