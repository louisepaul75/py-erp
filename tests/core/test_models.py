...
class TestAuditLogModel(TransactionTestCase):
    ...
    def test_audit_log_str_method(self):
        ...
        log_no_user = AuditLog.objects.create(
            event_type=AuditLog.EventType.SYSTEM_ERROR, message="System error occurred"
        )
        expected_str = (
            f"{log_no_user.get_event_type_display()} - " f"{log_no_user.timestamp}"
        )
    ...
    def test_additional_data_storage(self):
        ...
        test_data = {
            ...
            "metadata": {"source": "admin interface", "session_id": "12345"},
        }
        ...
        self.assertEqual(
            retrieved_log.additional_data["fields_changed"], ["name", "email"]
        )
        self.assertEqual(
            retrieved_log.additional_data["metadata"]["source"], "admin interface"
        )
    ...
    def test_event_type_choices(self):
        ...
        for event_type in event_types:
            ...
            log = AuditLog.objects.create(
                event_type=event_type, message=f"Test for {event_type}"
            )
            ...
            self.assertEqual(AuditLog.objects.filter(event_type=event_type).count(), 1)
